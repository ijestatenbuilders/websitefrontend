 ...
 * - 'light': Soft, dull, calm pastel ice-blue translucent water/glass jelly that seamlessly blends into the bright hero
 * - 'dark': Radiant bioluminescent glowing electric cyan/azure jelly with vibrant highlights, neon rim, and sparkling spores
 */
export default function RealEstate3DCanvas({ mousePos, scrollY, theme = 'light' }) {
  ...
  // Parallax and Mouse Target Refs for smooth 60/120fps physics
  ...
  useEffect(() => {
    ...
    const ribbonGeo = new THREE.TorusKnotGeometry(4.2, 0.85, 200, 44, 2, 3);
    ...
    // 2. Secondary Left Jelly Ring
    const leftRingUniforms = {
      uTime: { value: 10 },
      uMouse: { value: new THREE.Vector2(0, 0) },
      uDistort: { value: 0.28 },
    ...
    const leftRingGeo = new THREE.TorusGeometry(3.0, 0.55, 56, 110);
    const leftRingMat = new THREE.ShaderMaterial({
      vertexShader: jellyVertexShader,
      fragmentShader: jellyFragmentShader,
      uniforms: leftRingUniforms,
      transparent: true,
      depthWrite: false,
      blending: THREE.NormalBlending,
      side: THREE.DoubleSide
    ...
    const leftJellyRing = new THREE.Mesh(leftRingGeo, leftRingMat);
    ...
    blobConfigs.forEach((cfg, idx) => {
      ...
      const geo = new THREE.IcosahedronGeometry(cfg.size, 28);
    ...
    for (let i = 0; i < sporeCount; i++) {
      ...
      const c = Math.random();
      if (c > 0.6) {
        sporeColors[i * 3] = 0.22; sporeColors[i * 3 + 1] = 0.74; sporeColors[i * 3 + 2] = 0.97; // #38bdf8
      } else if (c > 0.3) {
        sporeColors[i * 3] = 0.40; sporeColors[i * 3 + 1] = 0.91; sporeColors[i * 3 + 2] = 0.99; // #67e8f9
      } else {
        sporeColors[i * 3] = 0.58; sporeColors[i * 3 + 1] = 0.77; sporeColors[i * 3 + 2] = 0.99; // #93c5fd
      }
    ...
    const animate = () => {
      ...
      // Ribbon with lively cursor follow
      ...
      // Left Ring with dynamic counter-twist
      ...
      // Blobs with organic cursor floating
